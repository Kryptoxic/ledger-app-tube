...
static unsigned long monero_crc32(unsigned long inCrc32, const void *buf, size_t bufLen) {
    unsigned long crc32;
 ...
 */
static void monero_set_word(unsigned int n, unsigned int idx, unsigned int w_start,
                            unsigned char *word_list, int len) {
    while (w_start < idx) {
        len -= 1 + word_list[0];
        if (len < 0) {
            monero_clear_words();
...
int monero_apdu_manage_seedwords() {
    ...
    switch (G_monero_vstate.io_p1) {
        ...
        case 1:
            ...
            for (int i = 0; i < 8; i++) {
                unsigned int val = (seed[i * 4 + 0] << 0) | (seed[i * 4 + 1] << 8) |
                ...
                for (int wi = 0; wi < 3; wi++) {
                    if ((wc[wi] >= w_start) && (wc[wi] < w_end)) {
                        monero_set_word(i * 3 + wi, wc[wi], w_start,
                                        G_monero_vstate.io_buffer + G_monero_vstate.io_offset,
                                        MONERO_IO_BUFFER_LENGTH - G_monero_vstate.io_offset);
                    }
                }
            }
                ...
                for (int i = 0; i < 24; i++) {
                    for (int j = 0; j < G_monero_vstate.io_p2; j++) {
                        G_monero_vstate.io_buffer[i * G_monero_vstate.io_p2 + j] =
                            N_monero_pstate->words[i][j];
                    }
                }
                w_start =
                    monero_crc32(0, G_monero_vstate.io_buffer, G_monero_vstate.io_p2 * 24) % 24;
